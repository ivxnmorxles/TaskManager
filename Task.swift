...
extension Task {
    ...
    func save() {
        ...
        if let i = existingTasks.firstIndex(where:{ $0.id == self.id }){
            //existingTasks[i] = self
            existingTasks.remove(at: i)
            existingTasks.insert(self, at: i)
        }else{
           //existingTasks[i] = self
           //existingTasks[i].completedDate = self.completedDate
